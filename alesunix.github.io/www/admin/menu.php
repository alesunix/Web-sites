...
else
{
	...
	if(isset($_GET['menu']))
	{
		...
		$content = '
		<p><i class="icon-chevron-left"></i>  <a href="' . $config['sitelink'] . 'admin/menu.php">Вернуться к списку меню</a></p>
	...
	{
		$content = '
		<p>Здесь можно редактировать меню.</p>
		<h2>Список меню</h2>
		...
		foreach($vars2 as $i=>$var)
		{
			...
			{
				$vars[$var] = $$var;
				$content.= '<li><i class="icon-chevron-right"></i>  <a href="'.$config['sitelink'].'admin/menu.php?menu='.$var.'">'.$var.'</a></li>';
			}
		}
		$content .= '</ul><h2>Добавить новое меню</h2>';
	}
# Шаблон формы
	ob_start();
	include_once $path . 'template/menu.inc.php';
